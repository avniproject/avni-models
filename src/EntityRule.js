...
class EntityRule {
  ...
  getDecisions(entity, context) {
    ...
    const decisions = this._safeInvokeRule(
      this.decisionFn,
      "Decision",
      defaultValue,
      entity,
  ...
  getEnrolmentSummary(enrolment, context) {
    const defaultValue = [];
    const enrolmentSummary = this._safeInvokeRule(
      this.enrolmentSummaryFn,
      "Summary",
      defaultValue,
      enrolment,
      context
  ...
  _safeInvokeRule(func, ruleNameForLogging, defaultValue, ...params) {
    General.logInfo(
      "EntityRule",
      `Invoking rule ${ruleNameForLogging} on entity: ${params[0].constructor.name}`
    );
    ...
    if (_.isNil(results)) {
      General.logInfo(
        "EntityRule",
        `${ruleNameForLogging} rule didn't return anything for: ${params[0].constructor.name}`
      );
      return defaultValue;
    } else if (typeof results !== typeof defaultValue) {
      General.logInfo(
        "EntityRule",
        `${ruleNameForLogging} didn't return an array for: ${params[0].constructor.name}`
      );
      return defaultValue;
    }
    return results;
  ...
  validate(entity, form) {
    const validationResults = this._safeInvokeRule(
      this.validationFn,
      "Validation",
      [],
      entity,
    ...
    validationResults.forEach(
      (validationResult) => (validationResult.formIdentifier = BaseEntity.fieldKeys.EXTERNAL_RULE)
    );
    return validationResults;
  }

  getNextScheduledVisits(entity, config) {
    const nextScheduledVisits = this._safeInvokeRule(
      this.getNextScheduledVisitsFn,
      "NextScheduledVisits",
      [],
      entity,
      config
